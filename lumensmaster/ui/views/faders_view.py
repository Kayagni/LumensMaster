...
Affiche une grille configurable de faders verticaux (64 par défaut, 2 lignes).
Chaque fader comprend : numéro, slider vertical, valeur, label du contenu.
...
Interactions :
    - Drag slider     → pilotage direct du fader
    - Clic numéro     → sélectionne le fader
    - Ctrl+clic       → ajoute/retire de la sélection
    - Shift+clic      → sélection par plage
    - Molette          → +1/-1 sur les faders sélectionnés
    - Saisie valeur   → applique aux faders sélectionnés
    - Full (F) / Zero (Z)  → raccourcis
"""
...
class FadersView:
    """Fenêtre flottante affichant les faders submasters et le Grand Master."""
    ...
    def __init__(self, engine: Engine) -> None:
        ...
        # Sélection de faders (gérée côté vue)
        ...
        self._fader_widgets: dict[int, dict] = {}  # fader_id -> {select_btn, slider, value_text, label_text}
        ...
        self._theme_fader_selected: int = 0
    ...
    def build(self) -> int:
        """Construit la fenêtre flottante des faders."""
        self._create_themes()

        window_width = min(
            self._columns * (self._fader_width + 6) + 120,
            1400,
        )
        window_height = self._slider_height + 200
        ...
        with dpg.group(parent=self._window_id):
            ...
            with dpg.group(horizontal=True):
                ...
                # Séparateur visuel
                dpg.add_child_window(width=2, height=self._slider_height + 60,
    ...
    def _build_category_buttons(self) -> None:
        with dpg.group(horizontal=True):
            ...
            self._category_buttons["layout"] = self._icons.image_button(
                icon_name="btn_Faders_Layout_Cfg",
    ...
    def _build_fader(self, fader_id: int) -> None:
        ...
        with dpg.group():
            # Bouton de sélection (numéro du fader)
            selected = fader_id in self._selection
            sel_label = f"F{fader_id}"
            widgets["select_btn"] = dpg.add_button(
                label=sel_label,
                width=self._fader_width,
                callback=self._on_fader_select,
                user_data=fader_id,
            )
            sel_theme = self._theme_select_btn_on if selected else self._theme_select_btn
            dpg.bind_item_theme(widgets["select_btn"], sel_theme)
    ...
    def _rebuild_gm(self) -> None:
        ...
        with dpg.group(parent=self._gm_container):
            ...
            gm_value = self._format_value(self._engine.grand_master.level)
    ...
    def _create_themes(self) -> None:
        ...
        # Bouton sélection fader (sélectionné)
    ...
    def _select(self, fader_id: int) -> None:
        self._selection = {fader_id}
        self._last_selected = fader_id
        self._update_selection_display()

    def _select_add(self, fader_id: int) -> None:
    ...
    def _select_range(self, fader_id: int) -> None:
        if self._last_selected == 0:
            self._select(fader_id)
            return
    ...
    def _update_fader_display(self, fader_id: int) -> None:
        ...
        # Slider
        if dpg.does_item_exist(widgets["slider"]):
        ...
        # Bouton sélection
        if dpg.does_item_exist(widgets["select_btn"]):
            selected = fader_id in self._selection
            theme = self._theme_select_btn_on if selected else self._theme_select_btn
            dpg.bind_item_theme(widgets["select_btn"], theme)

    def _update_selection_display(self) -> None:
        for fader_id in self._fader_widgets:
            widgets = self._fader_widgets[fader_id]
            if dpg.does_item_exist(widgets["select_btn"]):
                selected = fader_id in self._selection
                theme = self._theme_select_btn_on if selected else self._theme_select_btn
                dpg.bind_item_theme(widgets["select_btn"], theme)
    ...
    def _on_fader_select(self, sender: int, value: Any, user_data: int) -> None:
        ...
        if ctrl:
            self._select_add(fader_id)
        elif shift:
            self._select_range(fader_id)
        else:
    ...
    def _on_slider_move(self, sender: int, value: int, user_data: int) -> None:
        ...
        widgets = self._fader_widgets.get(fader_id)
        if widgets and dpg.does_item_exist(widgets["value_text"]):
    ...
    # --- API externe (pour mise à jour depuis MIDI, etc.) ---
